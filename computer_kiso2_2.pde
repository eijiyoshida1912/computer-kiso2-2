...
void setup() {
    ...
    for (int i = 0; i < allRealEstateDataArray.size(); i++) {
        ...
        // 重複市区町村を入れない
        if (i == 0 || !allRealEstateDataArray.getJSONObject(i - 1).getString("Municipality").equals(allRealEstateDataArray.getJSONObject(i).getString("Municipality"))) {
            MunicipalityArray.setString(i, allRealEstateDataArray.getJSONObject(i).getString("Municipality"));
        }
    }
    // 重複市区町村を削除
    // for (int i = 0; i < MunicipalityArray.size(); i++) {
    //    MunicipalityArray.remove(i);
    /*
    if (i == 0) {
    continue;
} else if (MunicipalityArray.getString(i - 1).equals(allRealEstateDataArray.getJSONObject(i).getString("Municipality"))) {
    MunicipalityArray.remove(i);
} else {
    continue;
}
...
// }
    ...
    if (i == 0) {
    continue;
} else if (MunicipalityArray.getString(i - 1) == MunicipalityArray.getString(i)) {
    */
    //  MunicipalityArray.remove(i);
// }
